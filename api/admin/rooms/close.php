...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, status FROM raid_rooms WHERE id = ?");
    $stmt->execute([$roomId]);
    ...
    if ($room['status'] === 'closed') {
        echo json_encode(["success" => false, "message" => "ห้องนี้ถูกปิดไปแล้ว"]);
        exit;
    }

    // ✅ อัปเดตสถานะห้องเป็น closed
    $stmt = $pdo->prepare("UPDATE raid_rooms SET status = 'closed' WHERE id = ?");
    $stmt->execute([$roomId]);

    echo json_encode([
        "success" => true,
        "message" => "ปิดห้องเรียบร้อย",
        "data" => [
            "room_id" => $roomId,
