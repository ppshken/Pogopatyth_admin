...
export default function DashboardUserLeaders() {
  ...
  async function load() {
    try {
      ...
      const url = new URL(
        `${API_BASE}/api/admin/dashboard/dashboard_user_stats.php`,
      );
  ...
  const creators = useMemo(
    () =>
      (data?.top_creators ?? []).map((x) => ({
        avatar: x.avatar,
        name: x.username,
        value: x.rooms_created,
      })),
    [data],
  );
  const joiners = useMemo(
    () =>
      (data?.top_joiners ?? []).map((x) => ({
        avatar: x.avatar,
        name: x.username,
        value: x.rooms_joined,
      })),
    [data],
  );
  const ratings = useMemo(
    () =>
      (data?.top_host_ratings ?? []).map((x) => ({
        avatar: x.avatar,
        name: x.username,
        value: x.avg_rating,
        reviews: x.reviews,
    ...
    <div className="mt-8">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <li key={x.user_id} className="flex justify-between">
                  <div className="flex items-center gap-2 mb-2">
                    <img
                      ...
                      src={x.avatar || "/default_avatar.png"}
                    />
                    <span>
                      {i + 1}. {x.username}
                    </span>
          ...
          {/* เข้าร่วมห้องมากสุด */}
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
            <div className="mb-2 text-sm font-medium">
              ผู้เข้าร่วมห้องมากสุด
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
            <div className="mb-2 text-sm font-medium">
              โฮสต์คะแนนเฉลี่ยสูงสุด (มีรีวิว ≥ 3)
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
